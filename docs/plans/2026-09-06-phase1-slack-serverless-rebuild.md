...
- 対象ADR：[ADR-001：LambdaとStrands AgentsによるSlackチャットボットの再構築](../adr/001-serverless-strands-chatbot.md)、[ADR-003：個人利用に合わせた本番移行と運用検証](../adr/003-personal-use-operations.md)
...
本番切り替え前の受け入れ試験、負荷試験、障害試験、月額上限の事前確定は、ADR-003により通常利用での検証と既存の費用速報へ置き換える。
...
ルート直下の旧モジュールと旧デプロイ経路は、PR14まで手を入れずに残す。
...
| 月間利用量と月額上限 | ADR-003により既存の費用速報へ置き換え |
| 30日の会話保持方針 | 13 |
| S3の追加 | Phase 1では扱わない |
...
- 条件付き更新で所有者と試行番号を確認し、Lambdaの残り実行時間に30秒を加えたリースを取得する
...
- 投稿済みの分割は再試行時に投稿せず、未投稿の分割だけを投稿する
- 分割投稿では分割位置と各tsを保存する
...
- モデル呼び出し回数、ツール呼び出し回数、出力トークン、スレッド当たり100回答の上限を実装する
- 回答上限後は新しいスレッドを促す案内を投稿し、モデルの出力上限到達時は生成済みの本文と打ち切り案内を返信する
...
- 削除：`src/*.py`の旧モジュール、旧デプロイ経路、対応する旧テスト
- 変更：`README.md`
...
**検証**：切り替え後、通常利用を続けながらアラートの発生状況を確認する。
