...
| 3 | 9 | 読み取りツール | 2 |
| 3 | 12 | デプロイパイプライン | 4 |
| 4 | 8 | ワーカーのオーケストレーション | 5, 6, 7 |
| 4 | 11 | 監視とログ | 4 |
| 5 | 10 | 会話サービス | 3, 6, 8, 9 |
| 6 | 13 | 受け入れ試験と切り替え手順 | 10, 11, 12 |
| 7 | 14 | 本番切り替えと旧実装の廃止 | 13 |
...
- ADR-001の「初期設定」の値をパラメーターとして外に出す
...
- チャンネル単位の投稿枠をDynamoDBの条件付き更新で予約する
...
- 相関ID、状態、所要時間、トークン数、ツール名、エラー分類を構造化ログとして出す
- 本文、取得したページ、添付、認証ヘッダー、署名、秘密値をログとトレースから除く
- ログの保持期間を14日に設定する

**検証**：意図的にDLQへ送り、通知が届くことを確認する。
ログ出力に本文が含まれないことをテストで固定する。
