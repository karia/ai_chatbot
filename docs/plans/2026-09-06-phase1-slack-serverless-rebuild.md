...
本番切り替え前の受け入れ試験、負荷試験、障害試験、月額上限の事前確定は、ADR-003により通常利用での検証と費用通知へ置き換える。
...
| 月間利用量と月額上限 | ADR-003により費用通知へ置き換え |
| 30日の会話保持方針 | 13 |
| S3の追加 | Phase 1では扱わない |
...
**目的**：本番切り替えに使う手順を文書として残し、費用の通知を用意する。

- 作成：`docs/runbooks/slack-cutover.md`
- 変更：`terraform/`
- 切り替え手順、ロールバック手順、DLQ復旧手順を書く
- AWS Budgetsの費用通知を作り、通知額と宛先の設定手順を残す
- 30日の会話保持方針を確定する
