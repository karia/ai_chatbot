...
旧実装のLambdaハンドラーはHTTPリクエスト内で推論とSlack返信を行い、受信確認が推論の完了を待っていた。
旧実装のイベント保存は作成済みイベントを重複として扱うため、途中失敗した処理の再送から安全に再開できなかった。
...
- 決定日：2026-09-06
- 採用案：[ADR-001](001-serverless-strands-chatbot.md#採用判断の記録)をAcceptedとする。
- 比較案：ADR-002はRejectedとする。

採用理由は次のとおりである。

- [コスト試算の仮定に基づく月額差](#両案の比較結果)は同じ利用量でSlackのみUSD 2.28、Discord込みUSD 3.52であり、運用対象を増やす理由としては小さい。
...
| AWS中心案 | Slack | 17.61 |
| ハイブリッド案 | Slack | 15.33 |
| AWS中心案とk3s Gateway | SlackとDiscord | 18.85 |
...
同じDiscord対応まで含めると、上記の仮定ではハイブリッド案に月額約USD 3.52の差が生じる。
この差からバックアップ費用と追加運用費を差し引いて判断する。
Slackだけなら差は約USD 2.28に縮まり、低利用量ではLambdaの費用削減だけを理由に運用対象を増やす効果は小さい。
差額は丸め前の数値で算出した。
