...
AWS側には推論、会話保存、処理状態、クラスタへ返すデータの転送を計上する。
...
アプリ、PostgreSQL、監視を加えた増分消費電力を5 W、電力単価を35円/kWhと仮定すると、`5 ÷ 1,000 × 720 × 35 = 126円/月`となる。
消費電力と電力単価には比較用の仮定を置いており、導入後に実測値と契約値へ置き換える。

| k3s側の費目 | 月額の扱い |
| --- | --- |
| 増分電力 | 126円 |
| PostgreSQLとMinIOのディスク | 既存容量内なら追加購入費0、増設時は償却費を加算 |
| Cloudflare TunnelとDNS | 既存の契約とドメインを利用し、増分請求0と仮定 |
| PrometheusとLoki | 既存基盤を共用し、増分を電力と容量に含める |
| 別障害領域へのバックアップ | 保存先の月額費用Bを加算 |
| 証明書更新と復旧作業 | 月間作業時間H × 時間単価Rを加算 |

本案のバックアップを省略してBを0とする計算は採用しない。
既存の別ホストへ退避でき、容量と契約に余裕がある場合だけ、追加請求額としてBを0にできる。
保存先の設備費や電力費を配賦する場合はBへ含める。
MinIOのファイル保存も追加する場合は、保存量とバックアップ要件に応じて容量を再評価する。

したがって月額は、約USD 14.43に126円とバックアップ等の実費を加えた金額となる。
比較のために1 USD＝150円という仮の換算率を置くと、`USD 14.42587 + 126 ÷ 150 = 約USD 15.27/月`にBと運用費を加える。
...
| k3sとAWSでSlackとDiscordを受付 | 15.27 | バックアップ、増設、追加運用の費用 |
...
同じDiscord対応まで含めると、上記の仮定ではハイブリッド案に月額約USD 17.85の差が生じる。
この差からバックアップ費用と追加運用費を差し引いて判断する。
Slackだけなら差は約USD 2.75に縮まり、低利用量ではLambdaやSQSの費用削減だけを理由に運用対象を増やす効果は小さい。
差額は丸め前の数値で算出した。
