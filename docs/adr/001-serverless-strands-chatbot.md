...
> [!NOTE]
> 本番切り替え前の受け入れ条件は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。

## 背景と対象範囲

旧実装のLambdaハンドラーは、HTTPリクエスト内で履歴取得、URL取得、推論、Slack返信を実行していた。
旧実装のイベント保存は作成済みのイベントを重複として扱うため、`processing`のまま失敗したイベントの再送でも処理を終了していた。
旧実装のSlack連携は呼び出しごとにSlackからスレッド履歴を取得し、URLや添付を再取得していた。
...
通常の会話、URL参照、テキスト添付を引き継ぎ、Slackスレッド当たりの回答上限を100回とする。
...
本ADRのバッチサイズ1、可視性1,800秒、ワーカー300秒、最大同時実行数5を使用する。
...
許可サーバーとチャンネル、通常投稿とメンションの選別、Message Content Intent、回答上限、参加者への保存案内は[ADR-002のDiscord受信設計](002-hybrid-k3s-strands-chatbot.md#discordの通常メッセージとメンション)と[参加者への案内](002-hybrid-k3s-strands-chatbot.md#参加者への案内)に従う。
...
| --- | --- |
| 受付Lambdaタイムアウト | 10秒 |
| ワーカーLambdaタイムアウト | 300秒 |
| モデル呼び出し上限 | 4回/イベント |
| ツール呼び出し上限 | 3回/イベント |
| 出力トークン上限 | 8,192/モデル呼び出し |
| ワーカーの予約済み同時実行数 | 5 |
...
各処理段階の前にLambdaの残り実行時間を確認する。
モデルまたはエージェントの回数・出力上限に達した場合は、生成済みの本文と打ち切り案内を返信する。
...
条件付き更新で所有者と試行番号を確認し、Lambdaの残り実行時間に30秒を加えたリースを確保する。
...
100回答に達した後のイベントには新しいスレッドを促す案内を投稿し、イベントを完了する。
この案内は回答数に加算しない。
...
> [!NOTE]
> 本番切り替え前の負荷試験と障害試験は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。
...
予算通知に加え、モデル呼び出し上限、Slackスレッドの100回答上限、同時実行上限、ワーカー停止手順を用意する。

> [!NOTE]
> 本番切り替え前の月間利用量と月額上限の確定は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。
...
> [!NOTE]
> 手順4の負荷試験と障害試験、および手順5の月間利用量と月額上限の確定は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。
...
> [!NOTE]
> 以下の表を本番切り替えの前提として全件検証する要件は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。
...
| 制限 | 実行予算、ツール回数、出力上限、Slackスレッドの100回答上限を守る |
...
> [!NOTE]
> 以下の表の「月間利用量と月額上限」の行は、個人利用を前提とする範囲に限り[ADR-003](003-personal-use-operations.md)で置き換える。
